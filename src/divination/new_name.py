...
class NewNameFactory(DivinationFactory):
    ...
    def build_prompt(self, divination_body: DivinationBody) -> tuple[str, str]:
        if (not divination_body.new_name or not all([
            ...
            raise HTTPException(status_code=400, detail="起名參數錯誤")
        ...
        prompt = (
            ...
            f"生日是{birthday.year}年{birthday.month}月{birthday.day}日{birthday.hour}時{birthday.minute}分{birthday.second}秒"
        )
